...
async function fetchAll() {
  ...
  try {
    const myArr = [
      fetchData("https://capsules7.herokuapp.com/api/carMarket/agencies"),
      fetchData("https://capsules7.herokuapp.com/api/carMarket/customers"),
      fetchData("https://capsules7.herokuapp.com/api/carMarket/tax"),
    ];
...
//? 3) Create two button on the screen "Customers" "Agencies"
...
function ListHandler(event) {
  ...
  const idsArr = [
    getId(set.dataAll.agencies),
    getId(set.dataAll.customers),
  ].flat();
  for (const Id of idsArr) {
    if (Id == targetId) {
      ...
      let parentCustomers =
        event.target.parentElement.classList.contains("customersList");
      let parentAgencies =
        event.target.parentElement.classList.contains("agenciesList");
      if (parentCustomers) {
...
async function fetchAgencyById(str) {
  const agency = await fetchData(
    `https://capsules7.herokuapp.com/api/carMarket/agencies/${str}`
  );
  return agency;
}
async function fetchCustomerById(str) {
  const customer = await fetchData(
    `https://capsules7.herokuapp.com/api/carMarket/customers/${str}`
  );
  return customer;
}
